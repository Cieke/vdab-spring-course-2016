...
public class JdbcFoodRepository implements FoodRepository{
    ...
    private RowMapper<Food> frm = (rs,i) ->{
        ...
        if(rs.getString("food_class").equals("MeatyFood")){
            ...
            f = new MeatyFood(rs.getInt("id"), name, calories, type);
        }
        else{
            boolean fruit =rs.getBoolean("fruit");
            boolean vegetables = rs.getBoolean("vegetables");
            f = new VegiFood(rs.getInt("id"), name, calories, fruit, vegetables);
        }
        return f;
    };
    ...
    public List<Food> findFoodForAnimalType(Animal.Type type) {
        return jt.query("select food.* FROM animal_diet left join food " +
                        "on food.id = animal_diet.food_id " +
                        "where animal_diet.animal_type=? order by food.name" , frm, type.name());
    }
    ...
    public void addFoodForAnimalType(Animal.Type animalType, Food food) {
        ...
        jt.update("insert into food(id, name, calories, food_class, meaty_food_type, fruit, vegetables) values(?,?,?,?,?,?,?)",
                food.getId(),
                food.getName(),
                food.getCalories(),
